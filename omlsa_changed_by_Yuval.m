function [y, out, sigma_v_squared]=omlsa_changed_by_Yuval(fin, fout, beta, Fs_ref, M_ref, overlap)
...
% The following code was changed by Yuval Konforti to:
% - have a kaiser window with some input beta parameter.
% - export the estimate of noise variance
% - Fs_ref, window length, and overlap to be given as input.

% 1) Parameters of Short Time Fourier Analysis: - Changed by Yuval Konforti
...
% window function
%win=hamming(M); --- Changed by Yuval Konforti.
win= kaiser(M, beta); 
...
finID = fopen(fin,'r');
sigma_v_squared= zeros(M21, Nframes); % --- Added by Yuval Konforti.
for l=1:Nframes
    if l==1
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
        ...
        sigma_v_squared(:, l)= lambda_d;% --- Added by Yuval Konforti.
